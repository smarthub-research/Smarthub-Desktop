...
async def perform_calibration(data: dict):
    ...
    # Fetch the smarthub device id
    smarthub_id = data["smarthub_id"]
    calibration_name = data["calibration_name"]
    with open("data.json", "w") as json_file:
        json.dump(data, json_file, indent=4)
    left_gain, right_gain, wheel_dist = fsolve(minimize_turnaround, [20,20,20], args=data)
    return await save_calibration(smarthub_id, data, left_gain, right_gain, wheel_dist, calibration_name)
...
def smooth_data(data):
    ...
    time_array = np.array(data['time_from_start'])
    if len(time_array) > 1:
        dt = (time_array[1] - time_array[0]) / 1000.0 if time_array[1] > 1000 else time_array[1] - time_array[0]
    else:
        dt = 0.1  # default sampling rate
